...
export function toSubtask(s: DbSubtask): Subtask {
  return {
    ...
    priority: (s.priority ?? "1") as Subtask["priority"],
    plannedStartTime: s.plannedStartTime ? s.plannedStartTime.toISOString() : null,
    plannedDueTime: s.plannedDueTime ? s.plannedDueTime.toISOString() : null,
    finishTime: s.finishTime ? s.finishTime.toISOString() : null,
