...
const toTaskPriorityLabel = (priority?: string | null) => {
...
const toTaskPriorityCode = (priorityText: string): "0" | "1" | "2" | "3" => {
...
const extractProjectNameReply = (text: string): string => {
...
const extractSubtaskDraft = (text: string): { title?: string; taskId?: string } => {
...
type UpdateTaskPriorityTarget = {
  raw: string;
  coreName: string;
  priority: "0" | "1" | "2" | "3";
};

type UpdateTaskDueTarget = {
  raw: string;
  coreName: string;
  dueAt: Date | null;
};

type MoveTaskToProjectTarget = {
  taskRaw: string;
  taskCoreName: string;
  projectRaw: string;
...
const extractUpdateTaskPriorityTarget = (text: string): UpdateTaskPriorityTarget | null => {
...
const extractUpdateTaskDueTarget = (text: string): UpdateTaskDueTarget | null => {
  const patterns = [
...
const extractMoveTaskToProjectTarget = (text: string): MoveTaskToProjectTarget | null => {
  const patterns = [
...
const extractModifyTaskIntentTarget = (text: string): { raw: string; coreName: string } | null => {
  const patterns = [
    /(?:修改|更新|调整)\s*任务\s*(.+?)\s*$/,
    /(?:修改|更新|调整)\s*(.+?)\s*任务\s*$/,
  ];
...
export class AiService {
  ...
  private pendingProjectCreateKey(ctx: AuthContext): string {
    return `${ctx.tenantId}:${ctx.userId}`;
  }
  ...
  private pendingConfirmActionKey(ctx: AuthContext): string {
    return `${ctx.tenantId}:${ctx.userId}`;
  }

  private pendingTaskModifyTargetKey(ctx: AuthContext): string {
    return `${ctx.tenantId}:${ctx.userId}`;
  }
  ...
  private async createTaskFromDraft(
    ...
    const project = await this.resolveProject(ctx, inputBizId, draft.projectName);
...
`.trim();
  ...
  async streamChat(
    ...
    try {
      ...
      const hasPendingConfirmAction = this.pendingConfirmActionMap.has(this.pendingConfirmActionKey(ctx));
  ...
  async chat(params: ChatParams, ctx: AuthContext): Promise<AiResponse> {
    try {
      ...
      const pendingProjectKey = this.pendingProjectCreateKey(ctx);
      ...
      const pendingConfirmKey = this.pendingConfirmActionKey(ctx);
      const pendingConfirm = this.pendingConfirmActionMap.get(pendingConfirmKey);
      const pendingModifyKey = this.pendingTaskModifyTargetKey(ctx);
      ...
      if (pendingConfirm) {
        ...
        if (isExpired) {
          this.pendingConfirmActionMap.delete(pendingConfirmKey);
        } else if (/^(继续|确认|确定|是|好的|好|ok|okay|yes)$/.test(normalized)) {
          this.pendingConfirmActionMap.delete(pendingConfirmKey);
          return this.confirmAction(pendingConfirm.action, pendingConfirm.params, ctx);
      ...
      if (pendingCreate) {
        ...
        if (!pendingCreate.title) {
          ...
          this.pendingTaskCreateMap.set(pendingKey, nextPending);
        ...
        if (!pendingCreate.bizId && !pendingCreate.projectName) {
          ...
          if (!dueAtCandidate && !noDueCandidate && !noProject) {
            ...
            if (projectName) {
              const project = await this.resolveProject(ctx, undefined, projectName);
              if (!project) {
                return {
                  success: true,
                  output:
      ...
      if (pendingProjectCreate) {
        if (/^(取消|算了|不用了|先不创建)/.test(question)) {
          this.pendingProjectCreateMap.delete(pendingProjectKey);
          this.pendingStructuredInputMap.delete(pendingStructuredKey);
          return {
            success: true,
      ...
      if (pendingSubtaskCreate) {
        if (/^(取消|算了|不用了|先不创建)/.test(question)) {
          this.pendingSubtaskCreateMap.delete(pendingSubtaskKey);
          return {
            success: true,
      ...
      if (isExplicitOperation) {
        ...
        if (modifyIntentTarget) {
          this.pendingTaskModifyTargetMap.set(pendingModifyKey, {
        ...
        if (/^(?:请|帮我|麻烦|可以)?\s*(?:创建|新建|建立|添加)\s*项目\s*$/.test(q)) {
          ...
          this.pendingProjectCreateMap.set(pendingProjectKey, { requestedAt: Date.now() });
          return {
            success: true,
        ...
        if (/^(?:请|帮我|麻烦|可以)?\s*(?:创建|新建|添加)\s*子任务\s*$/.test(q)) {
          this.pendingSubtaskCreateMap.set(pendingSubtaskKey, { requestedAt: Date.now() });
          return {
            success: true,
        ...
        if (statusTarget) {
          ...
          if (numericTarget) {
            ...
            this.pendingConfirmActionMap.set(pendingConfirmKey, {
        ...
        if (moveTarget) {
          ...
          const project = await this.resolveProject(ctx, undefined, moveTarget.projectCoreName);
          if (!project) {
            return {
              success: true,
...
- 所属项目: ${project?.projectName ?? "未归属项目"}`;
      ...
      // 命中了“事务型”语义，但未被任何确定性分支接住：明确告知“未执行”，避免模型自由发挥造成假成功
      if (isExplicitOperation) {
        return {
          success: true,
          output:
...
建议你把每一步再细化到“负责人 + 截止时间 + 交付物”。`;
  ...
  async confirmAction(action: string, params: any, ctx: AuthContext): Promise<AiResponse> {
    const startedAt = Date.now();
    this.pendingConfirmActionMap.delete(this.pendingConfirmActionKey(ctx));
